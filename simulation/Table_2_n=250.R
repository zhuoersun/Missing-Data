# 1. Simulations for normal data, 40% missing on average;
# 2. Sourced functions below should be in the same directory;
# 3. Sample size n=250, MC=1000 repeated simulations, random seed=1234;
# 4. X~N(0,1), Z~N_3(0,I_3), noise~N(0,1);
...
Beta0 <- full_MAR(xd="n",inno="n",alpha=alpha,n=250,seed=1234)
bias0 <- Beta0$bias
epsd0 <- Beta0$epsd
...
####CC under MAR####
Beta1 <- CC_MAR(xd="n",inno="n",alpha=alpha,n=250,seed=1234)
bias1 <- Beta1$bias
epsd1 <- Beta1$epsd
...
####IPW under MAR####
Beta2 <- ip_noaug_MAR(xd="n",inno="n",alpha=alpha,n=250,seed=1234,trim=1e-4)
bias2 <- Beta2$bias
epsd2 <- Beta2$epsd
...
####PIPA under MAR####
Beta3 <- ip_aug_MAR(xd="n",inno="n",alpha=alpha,n=250,seed=1234,trim=1e-4)
bias3 <- Beta3$bias
epsd3 <- Beta3$epsd
cover3 <- Beta3$cover
...
####AWEE under MAR####
Beta5 <- AWEE_MAR(xd="n",inno="n",alpha=alpha,n=250,seed=1234,trim=1e-4)
bias5 <- Beta5$bias
epsd5 <- Beta5$epsd
asysd5 <- (Beta5$asysd)/sqrt(MC*250)
cover5 <- Beta5$cover




save.image(file="MC_normal_n=250_3.RData")
